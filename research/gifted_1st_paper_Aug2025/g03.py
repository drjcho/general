...
# Load the dataset with original (0-6 scaled) AveAca and AveSocio values
data = pd.read_csv('./HOPE-Data-Kor-Master-data.csv')  # Replace with actual file path

# Group by Teacher ID and calculate summary statistics
teacher_stats = data.groupby('Teacher ID').agg(
    AveAca_Mean=('AveAca', 'mean'),
    AveAca_Std=('AveAca', 'std'),
    AveSocio_Mean=('AveSocio', 'mean'),
    AveSocio_Std=('AveSocio', 'std'),
    AveAca_Socio_Correlation=('AveAca', lambda x: x.corr(data.loc[x.index, 'AveSocio']))
).reset_index()

# Display the statistics to spot potential biases or inconsistencies
print("Teacher-Level Summary Statistics:")
print(teacher_stats)

# Visualization 1: Box plots of AveAca and AveSocio by Teacher ID
plt.figure(figsize=(12, 6))
sns.boxplot(x='Teacher ID', y='AveAca', data=data)
plt.title('Distribution of AveAca by Teacher ID')
plt.xticks(rotation=90)
plt.show()

plt.figure(figsize=(12, 6))
sns.boxplot(x='Teacher ID', y='AveSocio', data=data)
plt.title('Distribution of AveSocio by Teacher ID')
plt.xticks(rotation=90)
plt.show()

# Visualization 2: Correlation heatmap for each Teacher ID (optional)
# Using pairplot to visually compare AveAca and AveSocio by Teacher ID
sns.pairplot(data, hue="Teacher ID", vars=["AveAca", "AveSocio"])
plt.show()
